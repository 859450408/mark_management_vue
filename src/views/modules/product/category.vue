    ...
    //import引入的组件需要注入到对象中才能使用
      ...
      methods: {
        ...
        editCategory() {
          ...
          this.$http({
            url: this.$http.adornUrl("/markproduct/category/update"),
            method: "post",
            ...
            .then(({ data }) => {
              this.$notify({
                title: '成功',
                message: `修改${this.category.name}菜单成功`,
                type: 'success'
              });
              // 关闭对话框
        ...
        edit(data) {
          ...
          // 发送请求获取节点最新的数据
          this.$http({
            url: this.$http.adornUrl(`/markproduct/category/info/${data.catId}`),
            method: "get",
          }).then(({ data }) => {
            // 请求成功
        ...
        addCategory() {
          ...
          this.$http({
            url: this.$http.adornUrl("/markproduct/category/save"),
            method: "post",
            data: this.$http.adornData(this.category, false),
          }).then(({ data }) => {
              this.$notify({
                title: '成功',
                message: `保存${this.category.name}菜单成功`,
                type: 'success'
              });
              this.dialogVisible = false;
              // 刷新出新的菜单
        ...
        remove(node, data) {
          ...
          this.$confirm(`此操作将永久删除${data.name}菜单, 是否继续?`, '提示', {
            ...
            this.$http({
              url: this.$http.adornUrl('/markproduct/category/delete'),
              method: 'post',
              ...
              this.$notify.error({
                title: '错误',
                message: `删除${data.name}菜单错误`
              });
            });
          }).catch(() => {
            this.$notify.info({
              title: '消息',
              message: `取消删除${data.name}菜单成功`
            });
          });
        ...
        batchSave() {
          this.$http({
            url: this.$http.adornUrl("/markproduct/category/update/sort"),
            method: "post",
